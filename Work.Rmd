...
total_pos <- site_based_pos %>% 
  ...
  ungroup() %>% 
  mutate(total_state=sum(positive_status)) %>% 
  mutate(all_state=sum(singles))
...
species_site_plot <- posts_site %>% 
  ...
  geom_violin()+
  geom_boxplot(width=0.1,aes(fill=site), position=position_dodge(width=0.9), outlier.size=0)+
  geom_point(data=species_site_model$data, aes(y=virus, shape=species),
             position=position_dodge(width=0.9))+
scale_y_log10()+
  labs(y="Average Ranavirus Copies", x="Species")+
  theme(axis.text.x=element_text(size=8, angle=30, hjust=1))+
  guides(fill=guide_legend(title="Site"), shape=guide_legend(title="Species"))
species_site_plot
...
```{r}
#Adding 0.000001 to all average virus values so that I have no zero values and am able to use the Gamma distribution for my model
newest <- prevalence_data_site %>% 
  mutate(whole=(prevalence*100)) %>% 
  select(site, whole)
  
get_prior(whole~site, data=newest, family=Gamma(link="log"))

#model
modmod <- brm(whole ~ site,
                  family=Gamma(link="log"),
                  data=newest %>% filter(whole>0),
                  prior=c(prior(normal(0.7,0.3), class="b"),
                          prior(normal(-0.01, 0.05), class="Intercept")),
                  #sample_prior="only",
                  iter=1000, chains=1,
                  file="modmod.rds",
                  file_refit="on_change")

plot(conditional_effects(modmod, points=T))
