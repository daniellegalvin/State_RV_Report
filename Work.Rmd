...
site_based_pos <- data %>% 
  mutate(positive_status=case_when(result=="positive" ~ 1,
                            TRUE ~ 0))%>% 
  #filter(value_1>0) %>% 
  select(sample_label, site, side_of_state, species, value_1, value_2, value_3) %>% 
...
Model looking just at the relationship between average copies of virus and species.
```{r}
...
# 
# #Getting the necessary information for my priors including coefficients and classes
# get_prior(average_virus ~ species,
#           family=hurdle_gamma(link="log"), data=new_data)
# 
# 
# 
# species_model <- brm(virus ~ species,
...
#                   #sample_prior="only",
#                   iter=1000, chains=4,
#                   file="Models/species_model.rds",
#                   file_refit="on_change")
# 
# saveRDS(species_model, "Models/species_model.rds")
# 
...
# species_model
# 
# plot(conditional_effects(species_model, points=T))
# 
# cond_data <- distinct(species_model$data %>% select(-virus))
# 
# posts <- add_epred_draws(species_model, newdata=cond_data)
# 
# posts %>% 
#   ggplot(aes(x=species, y=.epred))+
#   geom_violin()+
#   geom_point(data=species_model$data, aes(y=virus),
#              position=position_dodge(width=0.9))+
#   geom_point(data=species_model$data, aes(y=(median(virus))), color="red", shape=17, size=2.5)+
#   geom_point(data=species_model$data, aes(y=(mean(virus))), fill="blue", shape=22, size=2.5)+
#   scale_y_log10()+
#   labs(y="Ranavirus Copies", x="Species", caption="Figure 1. Violin plot depicting the density of estimated ranavirus copies\n by species. Blue square represents the mean and red triangle\n represents the median")
# 
# 
# #Plot median rather than mean
# 
# species_model$data
# 
# bayes_R2(species_model)
# 
# pp_check(species_model, type="stat_grouped", group="species")
# 
# 
# 
# species_model %>% 
#   ggplot(aes(x=species, y=virus, color=site_name))+
#   geom_point()
# 

```

A better model looking at the relationship between average copies of virus and the interaction between species and site.
```{r}
#Adding 0.000001 to all average virus values so that I have no zero values and am able to use the Gamma distribution for my model
new_data <- site_based_pos %>% 
   mutate(virus=(average_virus+0.1))
...
species_site_plot <- posts_site %>% 
  ggplot(aes(x=species, y=.epred, fill=site))+
  geom_violin()+
...
species_diff %>% 
  median_qi(woodhousii_blanchardi)
...
This is a model looking at viral load by life stage - might look at this more later
```{r}
#Adding a column to my data so that developmental stage is consistent: 1 is adult 0 is juvenile
stage_pos <- data %>% 
  mutate(positive_status=case_when(result=="positive" ~ 1,
                            TRUE ~ 0))%>% 
  mutate(devel=case_when(stage=="adult" ~ 1,
                         TRUE ~ 0)) %>% 
  select(sample_label, site, side_of_state, species, value_1, value_2, value_3,devel) %>% 
  group_by(sample_label) %>% 
  mutate(average_virus=mean(c(value_1, value_2, value_3))) %>% 
  view()

get_prior(average_virus~devel, data=stage_pos, family=Gamma)

#A model based on stage
stage_model <- brm(average_virus ~ devel,
                   family=Gamma(link="log"),
                   data=stage_pos %>% filter(average_virus>0.000001),
                   prior=c(prior(normal(0,0.5), class="b")),
                   sample_prior="only",
                   iter=1000, chains=4,
                   file="Models/stage_model.rds",
                   file_refit="on_change")

saveRDS(stage_model, "Models/stage_model.rds")

plot(conditional_effects(stage_model, points=T))

stage_model <- update(stage_model, iter=2000, chains=4)
